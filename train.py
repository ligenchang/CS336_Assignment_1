...
def load_checkpoint(src, weights, optimizer, device):
    checkpoint = torch.load(src)
    for k in weights:
        weights[k].data.copy_(checkpoint['weights'][k].to(device))
    optimizer.load_state_dict(checkpoint['optimizer'])
    return checkpoint['iteration']

def main():
    # Hyperparameters (can be replaced with argparse for CLI)
    vocab_size = 10000
    context_length = 256
    d_model = 512
    d_ff = 1344
